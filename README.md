...
Optionally upload one or more **Original Reference Media** items, then select an **Initial Prompt** (or write your own), and click **Analyze**. You can run fully text-only (chat-style) or multimodal with media attached. For multi-media uploads, FrameLab includes per-media tags in the request payload and lets you rename those tags before submission. After generation, you can click **Edit output** to adjust the markdown result in a dialog.
